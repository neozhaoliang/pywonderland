...
implicitly used the following non-trivial result about Coxeter groups:
    ...
    the Tits cone (its the whole space V when W is finite), then the stabilizing
    subgroup of `v` is a parabolic subgroup. Especilly when `v` is in the
    fundamental domain, the stabilizing subgroup is generated by the simple
    reflections whose mirrors all contain `v`.
"""
