...
class BasePolytope(object):
    ...
    def get_edges(self):
        ...
        If the initial vertex v₀ lies on the i-th mirror then the
        reflection about this mirror fixes v₀ and there are no edges
        of type i. Else v₀ and its virtual image v₁ about this mirror
        generate a base edge e₀, the stabilizing subgroup of e₀ is
        generated by the simple reflection ρᵢ plus those simple reflections
        ρⱼ such that ρⱼ fixes v₀ and commutes with ρᵢ: ρᵢρⱼ=ρⱼρᵢ. Again
        we use Todd-Coxeter's procedure to get a complete list of word
        representations for the edges of type i and applyt them to e₀ to
    ...
    def get_faces(self):
        ...
        The composition of two reflections ρᵢ and ρⱼ is a rotation
        which fixes a base face f₀. The stabilizing subgroup of
        f₀ is generated by {ρᵢ, ρⱼ} plus those simple reflections ρₖ
        such that ρₖ fixes v₀ and commutes with both ρᵢ and ρⱼ.
        """
        for i, j in combinations(self.symmetry_gens, 2):
