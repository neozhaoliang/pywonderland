...
class BasePolytope(object):
    ...
    def get_edges(self):
        ...
        If the initial vertex v0 lies on the i-th mirror then the reflection
        about this mirror fixes v0 and there are no edges of type i. Else v0
        and its virtual image v1 about this mirror generate a base edge e, the
        stabilizing subgroup of e is generated by the simple reflection ρi plus
        those simple reflections ρj such that ρj fixes v0 and commutes with ρi:
        ρiρj=ρjρi. Again we use Todd-Coxeter's procedure to get a complete list
        of word representations for the edges of type i and applyt them to e to
        get all edges of type i.
        """
    ...
    def get_faces(self):
        ...
        which fixes a base face f. The stabilizing subgroup of f is generated
        by {ρi, ρj} plus those simple reflections ρk such that ρk fixes the
        initial vertex v0 and commutes with both ρi and ρj.
        """
        for i, j in combinations(self.symmetry_gens, 2):
    ...
    def get_coset_representatives(self, subgens, coxeter=True):
        """
        :param subgens: a list of generating words of the subgroup, e.g. [(0,), (1, 2)]
...
class Snub24Cell(Polychora):
    ...
    Its symmetric group is generated by three rotations {r, s, t}, where r = ρ0ρ1,
    s = ρ1ρ2, t = ρ1ρ3. A presentation is
    ...
    def transform(self, vector, word):
        """
        The generators are 0 for r=ρ0ρ1, 2 for s=ρ1ρ2, 4 for t=ρ1ρ3.
        """
        for g in word:
            if g == 0:
                vector = np.dot(vector, self.reflections[0])
